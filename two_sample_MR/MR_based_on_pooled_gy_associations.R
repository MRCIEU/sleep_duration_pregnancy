...
############################################################################################################################
#leave one out
MRplot<-function(gx,gy){
  gxgyforplot<-harmonise_data(exposure_dat = gx,outcome_dat = gy,action=1)
  res_loo<-mr_leaveoneout(gxgyforplot)
  p3<-mr_leaveoneout_plot(res_loo)
  a<-p3[[1]]
  return(a)
}

#d1<-MRplot(gxforMR,loss)
d2<-MRplot(gxforMR,stillbirth)
d3<-MRplot(gxforMR,miscarriage)
d4<-MRplot(gxforMR,gdm)
d5<-MRplot(gxforMR,hdp)
d6<-MRplot(gxforMR,pd)
d7<-MRplot(gxforMR,pt)
d8<-MRplot(gxforMR,lbw)
d9<-MRplot(gxforMR,hbw)
d10<-MRplot(gxforMR,bw)
