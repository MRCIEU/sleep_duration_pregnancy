...
(BMI !=.)|(INCOME !=.)|(PIH_PE !=.)|(PARITET_5 !=.)

mi set mlong
mi register imputed sleep MORS_ALDER AA1124 AA1126 SMOKE SMOKEFTHR ALCOHOL BMI INCOME PIH_PE PARITET_5
...
ALCOHOL (regress) BMI (ologit) INCOME (logit) PIH_PE (ologit) PARITET_5 = i.SMOKERPLC, dryrun
mi impute chained (mlogit) sleep (regress) MORS_ALDER (ologit) AA1124 (ologit) AA1126 (logit) SMOKE (logit) SMOKEFTHR (logit) ///
ALCOHOL (regress) BMI (ologit) INCOME (logit) PIH_PE (ologit) PARITET_5 = i.SMOKERPLC, add(100) rseed(1234)
...
mi estimate: logit PIH_PE sleepL MORS_ALDER i.AA1124 i.SMOKE i.ALCOHOL BMI i.INCOME PARITET_5
mi estimate: logit PIH_PE ib3.sleep MORS_ALDER i.AA1124 i.SMOKE i.ALCOHOL BMI i.INCOME PARITET_5
*tab PIH_PE _mi_m, mis
tab PIH_PE _mi_miss, mis

misstable patterns PIH_PE sleep MORS_ALDER AA1124 AA1126 SMOKE SMOKEFTHR SMOKERPLC ALCOHOL BMI INCOME PARITET_5, freq


*likelihood ratio test
gen sleep3=sleep==3
gen sleep4=sleep==4
mi estimate: logit PIH_PE sleepL MORS_ALDER i.AA1124 i.SMOKE i.ALCOHOL BMI i.INCOME PARITET_5 sleep3 sleep4 
